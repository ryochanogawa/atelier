...
## なぜATELIERなのか — Claude/Codexをそのまま使う場合との違い

### 課題: モノリシックなプロンプト

Claude CodeやCodexを直接使う場合、1つのプロンプトに全てを詰め込みます。

```
あなたはシニアエンジニアです。このプロジェクトの構成はこうで、
こういうルールに従って、この要件を分析して、
このフォーマットで出力してください...
```

この方法には3つの問題があります。

1. **再現性がない** — 同じ作業でも毎回プロンプトを書き直す。人によって品質がブレる
2. **関心が混在する** — 役割・知識・手順・制約・出力形式が1つのテキストに混ざり、どれかを変えると全体に影響する
3. **ワークフローが頭の中にある** — 「まず設計して、次に実装して、テストして、レビュー」という手順を人間が管理する

### 解決: Faceted Prompting + ワークフロー自動化

ATELIERは2つのアプローチでこれを解決します。

**1. プロンプトの関心分離（Faceted Prompting）**

1つの巨大プロンプトではなく、5つの独立したファセット（関心）に分離して管理します。

```
System:  Palette（誰として振る舞うか）
User:    Knowledge（何を知っているか）
         Instruction（何をするか）
         Contract（どう出力するか）
         Policy（どんな制約に従うか）
```

各ファセットは独立したYAML/MDファイルで管理され、自由に組み替えられます。

- **Palette**を差し替えれば、同じタスクを別の専門家視点で実行できる
- **Contract**を差し替えれば、同じ分析から別フォーマットの出力が得られる
- **Policy**をチーム共通で定義すれば、誰が実行してもコーディング規約が守られる
- **Knowledge**を1回書けば、全てのCommissionで再利用できる

これはプロンプトにおける**依存性注入（DI）**です。

**2. ワークフローの構造化（Commission + Stroke）**

複数のAI実行をYAMLで定義した順序で自動実行し、前のステップの出力を次のステップに自動で引き渡します。

```yaml
# spec-plan Commission: 要件定義 → 設計 → タスク生成
strokes:
  - name: requirements   # Palette: spec-writer
    outputs: [requirements]
  - name: design          # 入力: requirements の出力を自動で受け取る
    inputs: [requirements]
    outputs: [design]
  - name: tasks           # 入力: requirements + design
    inputs: [requirements, design]
```

- 各ストロークに**異なるMedium**（Claude/Codex/Gemini）を割り当てられる
- 実装系はgit **worktree**で隔離し、壊れたら捨てるだけ
- 仕様書系はworktreeなしでプロジェクト直下に直接書き出す

### まとめ

| | Claude/Codex単体 | ATELIER |
|---|---|---|
| プロンプト | 毎回1枚に全部書く | 5ファセットを組み合わせて自動合成 |
| ワークフロー | 人間が手順を管理 | YAMLで定義、自動で順番実行 |
| 出力品質 | AIとプロンプト次第でブレる | Contractで構造を強制、再現可能 |
| AI切り替え | プロンプトの書き直しが必要 | Medium設定を変えるだけ |
| 役割分担 | 1つのAIに全部お願い | Paletteでストロークごとに専門家を割り当て |
| コンテキスト引き継ぎ | コピペで手動 | Canvasで自動引き渡し |
| 変更の安全性 | プロジェクトに直接書き込み | Worktreeで隔離、merge/deleteで制御 |

---

## 目次

1. [なぜATELIERなのか](#なぜatelierなのか--claudecodexをそのまま使う場合との違い)
2. [Faceted Prompting](#faceted-prompting)
3. [アーキテクチャ](#アーキテクチャ)
4. [ディレクトリ構造](#ディレクトリ構造)
5. [クイックスタート](#クイックスタート)
6. [CLIコマンド一覧](#cliコマンド一覧)
7. [仕様駆動開発（Spec-Driven Development）](#仕様駆動開発spec-driven-development)
8. [CI/CD 統合](#cicd-統合)
9. [高度な実行機能](#高度な実行機能)
10. [Commission YAML の書き方](#commission-yaml-の書き方)
11. [Palette YAML の書き方](#palette-yaml-の書き方)
12. [Policy YAML の書き方](#policy-yaml-の書き方)
13. [Knowledge .md の書き方](#knowledge-md-の書き方)
14. [Instruction .md テンプレートの書き方](#instruction-md-テンプレートの書き方)
15. [Contract YAML の書き方](#contract-yaml-の書き方)
16. [ビルトインリソース一覧](#ビルトインリソース一覧)
17. [美術工房メタファー](#美術工房メタファー)
...
├── builtin/                # ビルトインリソース
│   ├── commissions/        # ワークフロー定義 (16個)
│   ├── palettes/           # ペルソナ定義 (20個)
│   ├── policies/           # ポリシー (10個)
│   ├── knowledge/          # ドメイン知識 (8個)
│   ├── instructions/       # タスク手順テンプレート (15個)
│   └── contracts/          # 出力規約 (11個)
...
# .atelier/studio.yaml
name: my-project
theme: biohazard          # UIテーマ（biohazard / codec）
mediums:
  - name: claude-code     # デフォルトMedium
    timeout: 300000
    allowEdit: true
palette_providers:        # Paletteごとに異なるMediumを割り当て
  reviewer:
    medium: codex
pipeline:                 # Git/PR自動化設定
  branch_prefix: "atelier/"
  commit_message_template: "atelier: {{task}}"
database:                 # DB接続（spec設計時のスキーマ参照用、読み取り専用）
...
# default Commission を実行（plan → implement → test → review → fix ループ）
...
| `atelier spec client "<description>"` | 顧客向け要件定義書を生成する（JSON出力） |
| `atelier spec client --output sheets` | 顧客向け要件定義書を Google Sheets に出力する |
| `atelier spec client --output slides` | 顧客向け要件定義書を Google Slides プレゼンテーションに出力する |
| `atelier spec client --spec <id>` | 既存仕様書の要件を元に顧客向け要件定義書を生成する |
| `atelier spec client --skip-requirements` | 既存の要件定義JSONを再利用し、出力のみ再実行する |
...
6 stroke（requirements → verify → design → verify → tasks → verify）で止まります。各フェーズ間に品質検証ゲートが設けられます。生成された仕様書をレビュー後、`atelier spec implement` で実装に進めます。
...
9 stroke（requirements → verify → design → verify → tasks → verify → implement → test → review）が一気に実行されます。各フェーズ間に品質検証ゲートが設けられ、問題がある場合は前ステップに自動で差し戻されます。
...
| 名前 | ストローク | 用途 |
|------|-----------|------|
| `default` | plan → implement → test → review → fix ループ | 汎用開発（修正ループ付き） |
| `spec-driven` | requirements → verify → design → verify → tasks → verify → implement → test → review | 仕様駆動開発（検証ゲート付き全フェーズ一気通貫） |
| `spec-plan` | requirements → verify → design → verify → tasks → verify | 仕様駆動開発（検証ゲート付き設計フェーズのみ） |
...
| 名前 | ストローク | 用途 |
|------|-----------|------|
| `client-requirements` | client-requirements | 顧客向け要件定義書の生成（Google Sheets/Slides向け） |
| `slide-composition` | slide-composition | 要件定義JSONからスライド構成プランの生成 |
...
|------|------|
| `architect` | アーキテクチャ設計 |
| `business-analyst` | 顧客向け要件定義書・業務フロー生成 |
| `coder` | コード実装 |
...
| 名前 | 内容 |
|------|------|
| `client-requirements` | 顧客向け要件定義書の生成手順 |
| `design` | 設計の手順 |
| `fix` | バグ修正の手順（反復時のファイル確認フロー付き） |
| `implement` | 実装の手順（セルフチェックリスト付き） |
| `plan` | 計画策定の手順（良い例・悪い例付き） |
| `review` | レビューの手順（良い例・悪い例付き） |
| `security-review` | セキュリティレビューの手順 |
| `slide-plan` | プレゼンテーション用スライド構成プランの生成手順 |
| `spec-design` | 技術設計書の生成手順 |
| `spec-requirements` | 要件定義書の生成手順 |
| `spec-tasks` | 実装タスクの生成手順（品質チェック基準付き） |
| `test` | テスト作成の手順 |
| `verify-design` | 技術設計書の要件トレーサビリティ検証 |
| `verify-requirements` | 要件定義書の曖昧性・矛盾・不足チェック |
| `verify-tasks` | 実装タスクのカバレッジ・依存関係・完了条件検証 |

### Contracts (11個)

| 名前 | 内容 |
|------|------|
| `client-requirements-output` | 顧客向け要件定義書の出力フォーマット（JSON） |
...
| `slide-plan-output` | スライド構成プランの出力フォーマット（JSON） |
