...
## Comparison with TAKT / TAKTとの比較

ATELIERは [TAKT](https://github.com/nrslib/takt) にインスパイアされつつ、独自の設計哲学を持ちます。

| Feature | TAKT | ATELIER |
|---------|------|---------|
| Language | TypeScript | TypeScript |
| Metaphor | Music (Piece/Movement) | Art Studio (Commission/Stroke) |
| Architecture | — | DDD + Hexagonal |
| Prompt Design | Faceted Prompting | Faceted Prompting |
| Providers | Claude, Codex, Cursor, Copilot | Claude Code, Codex, Gemini |
| Auth | Subscription-based | Subscription-based |
| Interactive Mode | `takt` | `atelier talk` (+/requirements, /analyze, /suggest) |
| Task Queue | `takt add/run` | `atelier task add/run` |
| Issue Integration | GitHub/GitLab | GitHub |
| Auto PR | Yes | Yes (`--auto-pr`) |
| CI/CD | `--pipeline` | `atelier pipeline run` |
| Eject | `takt eject` | `atelier technique eject` |
| Parallel Execution | Yes | Yes (`depends_on` graph) |
| Built-in Workflows | 4 | 9 (+ review-gate, requirements, test, legacy, docs) |
| Persona Format | Markdown | YAML + Markdown |
| Package Management | Repertoire | Repertoire |
| Docker | Yes | Yes (multi-stage) |
| Review Gate | — | Risk scoring + Policy engine + Auto-approval |
| Security Scan | — | npm audit + License scan + SBOM (CycloneDX) |
| Requirements Analysis | — | AI interview + Contradiction/Gap detection |
| Test Analysis | — | Coverage gap + E2E scenario generation |
| Legacy Analysis | — | Codebase structure + Migration planning |
| Doc Management | — | Freshness scoring + Auto-generation |
| Prompt Assist | — | Intent enhancement + Palette/Commission suggestion |

---

## Acknowledgements / 謝辞

Inspired by [TAKT](https://github.com/nrslib/takt) — an AI agent orchestration tool that pioneered the concept of subscription-based CLI agent composition and Faceted Prompting architecture.
